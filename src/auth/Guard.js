...
class AuthProvider extends React.Component {
  ...
  signin = (clbk, data) => {
    apiAuthHandler
      ...
      .then(async serverRes => {
        ...
        clbk(serverRes.data.loginStatus); // then ... exec the callback (leading to a redirect)
      })
  ...
  signout = clbk => {
    ...
    apiAuthHandler.post("user/signout").then(serverRes => {
      console.log("par ici", clbk);
      
      this.setState({ loginStatus: false }, () => clbk(this.state.loginStatus));
    });
  };
